 ...
 */
@Component
public class StopallCommand extends BotCommand {
	...
	public StopallCommand() {
		super("stopall",
				"Hiermit können Pokémon- und Raid-Meldungen deaktiviert werden und der Bot kann gestoppt werden.\nBenutze <code>/stop pokemon</code> oder <code>/stop</code> um nur die Pokémon-Meldungen zu deaktivieren.");
	}
	...
	public void execute(AbsSender absSender, User user, Chat chat, String[] arguments) {
		...
		String text = "Du hast */stopall* eingegeben.";
		// if (realUser.isTelegramActive() && arguments.length > 0 &&
		// arguments[0].equals("all")) {
		...
		// } else if (realUser.isTelegramActive() && arguments.length > 0 &&
		// arguments[0].equals("pokemon")) {
		// realUser.setShowPokemonMessages(false);
		// realUser = userService.updateOrInsertUser(realUser);
		...
		// realUser.setShowPokemonMessages(false);
		// realUser = userService.updateOrInsertUser(realUser);
		// text = "Es werden im Moment keine Pokémon mehr angezeigt. Zum Beendes
		// aller Nachrichten gib folgendes ein:\n/stopall";
		// }
